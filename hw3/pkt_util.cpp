...
namespace pku {
    int send_packet(
        ...
        if (rdt::rdt_sendto(sock_fd, pkt.data_p, pkt.data_size, dst_addr, dst_len) < 0) {
    ...
    int send_int64(
        ...
        if (send_packet(sock_fd, pkt, dst_addr, dst_len) == 1) {
            goto send_int64_err;
        }
        // Send message
        pack_int64(num, pkt);
    ...
    int recv_int64(
        ...
        if (recv_packet(sock_fd, pkt, sizeof(uint64_t), src_addr, src_len) == 1) {
            goto recv_int64_err;
        }
        unpack_uint64(num_len, pkt);
        // Receive message
        if (recv_packet(sock_fd, pkt, sizeof(int64_t), src_addr, src_len) == 1) {
            goto recv_int64_err;
        }
    ...
    int recv_str(
        ...
        if (recv_packet(sock_fd, pkt, msg_len*sizeof(char), src_addr, src_len) == 1) {
            goto recv_str_err;
        }
        unpack_str(msg, pkt);
    ...
    int64_t recv_frame(
        ...
        // Receive frame
        if (recv_packet(sock_fd, pkt, img_size, src_addr, src_len) == 1) {
            return -1;
        }
    ...
    int send_video(
        ...
        if (send_packet(sock_fd, pkt, receiver_addr, receiver_len) == 1) {
            goto send_video_err;
        }
        ...
        while (1) {
            cap >> server_img;
            int64_t ret = send_frame(sock_fd, server_img, pkt, receiver_addr, receiver_len);
            if (ret < 0) {
                goto send_video_err;
    ...
    int recv_video(
        ...
        while (1) {
            ...
            if (send_packet(sock_fd, pkt, sender_addr, sender_len) == 1) {
                goto recv_video_err;
            }
            if (key_pressed == 27)
                break;

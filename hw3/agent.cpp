/* NOTE: The sample code only provides packet formats. No congestion control or
 * go-back-N checking. */
/* NOTE: UDP socket is connectionless. Thus, the target IP and port should be
 * specified whenever sending a packet. */
/* HINT:  We suggest using "bind," "sendto," and "recvfrom" in system/socket.h.
 */

/*
 * Connection Rules:
 * In this homework, each "agent," "sender," and "receiver" will bind a UDP
 * socket to their ports, respectively, for sending and receiving packets. When
 * "agent" receives a packet, it will identify whether the packet sender is
 * "sender" or "receiver" by the packet sender's IP and port. When "sender" or
 * "receiver" receives a packet, the packet sender is always "agent." When
 * "sender" or "receiver" wants to send a packet, they should know the IP and
...
int main(int argc, char *argv[]) {
    ...
    if (argc != 5) {
        fprintf(stderr,
                "Usage: %s <agent port> <sender IP>:<sender port> <receiver "
                "IP>:<receiver port> <error_rate>\n",
                argv[0]);
    ...
    printf("Start!! ^Q^\n");
    printf(
        "sender info: ip = %s port = %d and receiver info: ip = %s port = %d\n",
    ...
    while (1) {
        ...
        segment_size = recvfrom(agentsocket, &s_tmp, sizeof(s_tmp), 0,
                                (struct sockaddr *)&tmp_addr, &tmp_size);
        if (segment_size > 0) {
            inet_ntop(AF_INET, &tmp_addr.sin_addr.s_addr, ipfrom,
                      sizeof(ipfrom));
            ...
            if (strcmp(ipfrom, sendIP) == 0 && portfrom == sendPort) {
                ...
                if (s_tmp.header.fin == 1) {
                    printf("get\tfin\n");
                    sendto(agentsocket, &s_tmp, segment_size, 0,
                           (struct sockaddr *)&receiver, recv_size);
                    printf("fwd\tfin\n");
                } else {
